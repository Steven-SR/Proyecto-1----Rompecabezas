...
public class Tablero {
    ...
    public void createTablero() {
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                if (i == 0 && j == 0) {
                    tablero[i][j] = new Pieza(random.nextInt(rangoNum), random.nextInt(rangoNum),
                            random.nextInt(rangoNum), random.nextInt(rangoNum));
                }
                // Primera fila, solo depende de la pieza anterior
                else if (i == 0) {
                    tablero[i][j] = new Pieza(random.nextInt(rangoNum), random.nextInt(rangoNum),
                            random.nextInt(rangoNum), tablero[i][j - 1].getRight());
                }
                // Primera columna, solo depende de la pieza superior
                else if (j == 0) {
                    tablero[i][j] = new Pieza(tablero[i - 1][j].getDown(), random.nextInt(rangoNum),
                            random.nextInt(rangoNum), random.nextInt(rangoNum));
                }
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                ...
                // Revisa el resto de piezas
                    ...
                    if (tablero[i][j].getLeft() != tablero[i][j - 1].getRight()
                            || tablero[i][j].getUp() != tablero[i - 1][j].getDown()) {
                        return false;
                    }
                }
            }
    ...
    // Metodos para modificar el tablero, coloca una pieza en una posicion
    // especifica
